    ...
    <div className="min-h-screen bg-background" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <Suspense fallback={<div className="text-center py-20 opacity-50">Loading...</div>}>
          ...
          {activeTab === "tables" && <TablesTab tr={tr} activeTables={useMemo(() => tablesRaw.map(t => ({ ...t, userCount: users.filter(u => u.tableNumber === t.number).length, status: users.some(u => u.tableNumber === t.number) ? "occupied" : "available" })).sort((a,b) => a.number-b.number), [tablesRaw, users])} users={users} />}
