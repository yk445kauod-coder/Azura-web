...
export default function Welcome() {
  ...
  const handleGuestLogin = async () => {
    ...
    if (!name.trim()) {
      setError(tr("Please enter your name", "يرجى إدخال اسمك")); return;
    }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-5 py-8 relative overflow-hidden"
      ...
      <div className="w-full max-w-sm page-enter">
        ...
        <div className="card-elevated rounded-2xl p-6 space-y-4">
          ...
          >
            {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><Coffee size={18} /> {tr("Start Ordering", "ابدأ الطلب")}</>}
          </button>
        </div>
