...
export function parseFacebook(url: string): ParsedVideo | null {
  if (!url.includes("facebook.com") && !url.includes("fb.watch") && !url.includes("fb.com") && !url.includes("fbcdn.net") && !url.includes("web.facebook.com")) return null;
  
  // Video URLs - multiple patterns
  const videoMatch = url.match(/(?:facebook\.com|web\.facebook\.com)\/([a-zA-Z0-9.-]+)\/videos\/([a-zA-Z0-9]+)/);
  const reelMatch = url.match(/(?:facebook\.com|web\.facebook\.com)\/reel\/([a-zA-Z0-9]+)/);
  const watchMatch = url.match(/fb\.watch\/([a-zA-Z0-9_-]+)/);
  const postVideoMatch = url.match(/(?:facebook\.com|web\.facebook\.com)\/([a-zA-Z0-9.-]+)\/posts\/([a-zA-Z0-9]+)/);
  const storyMatch = url.match(/(?:facebook\.com|web\.facebook\.com)\/stories\/([a-zA-Z0-9]+)/);
  const videoIdMatch = url.match(/video_id=([a-zA-Z0-9]+)/);
  const vMatch = url.match(/(?:facebook\.com|web\.facebook\.com)\/[^\/]+\/v\/([a-zA-Z0-9]+)/);
  ...
  if (videoId || reelMatch || watchMatch || videoMatch) {
    // For Reels, use the direct embed approach
  ...
  // Generic Facebook URL - try to embed as post
  return {
    provider: "facebook",
    embedUrl: `https://www.facebook.com/plugins/post.php?href=${encodeURIComponent(url)}`,
    thumbnail: "",
...
export function getEmbedHtml(video: ParsedVideo, width = "100%" as string | number, height = "100%" as string | number): string {
  switch (video.provider) {
    ...
    case "facebook":
      // For Facebook, we need to load the SDK and use the proper embed
      return `<div id="fb-root-${Date.now()}"></div>
<script async defer src="https://connect.facebook.net/en_US/sdk.js#xfbml=1&version=v18.0" onload="window.dispatchEvent(new Event('fb-sdk-ready'))"></script>
<div class="fb-video" data-href="${video.embedUrl}" data-width="${width}" data-show-text="false" data-lazy="true" data-autoplay="true"></div>`;
